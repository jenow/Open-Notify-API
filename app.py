...
################################################################################
# ISS Locations between 2 dates
################################################################################
@app.route("/iss-between-dates.json")
@app.route("/iss-between-dates/")
@app.route("/iss-between-dates/v1/")
@jsonp
@json
def iss_between_dates():
    """The International Space Station (ISS) is moving at close to 28,000 km/h so its
    location changes really fast! Where is it right now?
    ...
    **Example response**:
    ...
    loc = iss.get_location_between_dates(start, end, interval)
    return dict({'message': "success"}, **loc), 200
